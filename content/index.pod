...
Please use the round robin DNS on L<irc.xinutec.net|irc://irc.xinutec.net>.
IPv6 is also supported via this host name. We currently still support
unencrypted connections via port 6667, but will soon only allow SSL connections
via port 7776. Web-based access to the network is available with
L<qwebirc|http://qwebirc.xinutec.net/> via IPv4.

=head2 Channels

C<#linux> is the main channel. If you run a channel, you are free to host a
channel homepage (static HTML only) on this server. To do this, simply fork our
L<github repository|https://github.com/xinutec/xinutec.github.io/> and create a
pull request once your changes are is in place. A staff member will then review
and approve it.

=head2 People

These are the network operators/staff (effective or retired) with links to their
homepages:
